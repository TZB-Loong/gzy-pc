 ...
 *  @param {boolean} visible true/fasls 弹框显示
 *  @param {function} changeModal   改变 visible
...
export default class SelectSupplier extends Component {
  ...
  componentDidMount() {
    ...
    //默认设置的值(初始化的时候)
    const { visible, tenderType } = this.props;
    this.initialization();
    this.setState({
  ...
  componentWillUpdate(nextProps) {
    // console.log(this.props.id,'this.props.id')
    //请求完成之后再次渲染
    const { defaultValue, visible, tenderType, isAuthentication, isPurchased } = this.props;
    if (nextProps.visible != visible && nextProps.visible) {
      ...
      // console.log(this.state.allChecked,defaultValue,'defaultValue')
    }
    // console.log(nextProps.isAuthentication,'0-0-0-')

    if(nextProps.isAuthentication!=this.props.isAuthentication){
    ...
    if (nextProps.isPurchased != this.props.isPurchased) { //当修改时
      this.setState({
  ...
  Ok = () => {
    //响应确定函数
    this.props.changeModal(false); // 关闭modal(弹框)
    const {
      pingtaiParamsCheckdList,
      allParamsCheckedList,
      pingtaiChecked,
            ...
            <Row className={Styles.search}>
              <Col span={12}>
                <Radio.Group value={this.state.bank} onChange={this.handleBankChange}>
                  ...
                  <Radio.Button value="all">向平台公开招采</Radio.Button>
                </Radio.Group>
              </Col>
              {
