...
@connect(({ labourSupplier, loading }) => ({
  labourSupplier,
  loading: loading.effects['labourSupplier/bizObjectList'],
}))
class AddfileLabour extends Component {
  state = {
    listData: [], //列表数据
    visible1: false, //弹框显示
    value: '',
  ...
  bizObjectList = () => {
    ...
    dispatch({
      ...
      let { labourSupplier, loading } = this.props;
      if (!loading) {
        let bizObjectList = labourSupplier.bizObjectList;
        this.setState({
  ...
  bizObjectEdit = data => {
    ...
    dispatch({
      type: 'labourSupplier/bizObjectMetadataEdit',
      payload: data,
    }).then(() => {
      _this.bizObjectList();
    });
  ...
  bizObjectDelete = data => {
    ...
    const { dispatch } = this.props;
    dispatch({
      type: 'labourSupplier/bizObjectMetadataDelete',
      payload: { id: data },
    }).then(() => {
      this.bizObjectList();
    });
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((errors, values) => {
      ...
      if (_this.state.show == 'add') {
        //添加
        let resquestData = { chnName: trim(values.input, 'g'), bizId: labourSupplier.bizId };
    ...
    return (
      <Spin spinning={this.props.loading}>
        <Card
          title={
            <Breadcrumb>
              <Breadcrumb.Item>{labourSupplier.data}</Breadcrumb.Item>
              <Breadcrumb.Item>
                <Link to="/supplierManagement/labour">劳务供应商首页</Link>
              </Breadcrumb.Item>
            </Breadcrumb>
          }
        >
...
export default Form.create()(AddfileLabour);
